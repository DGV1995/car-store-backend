...
def _create_test_image(format: str = "jpeg") -> io.BytesIO:
    ...
    if format == "jpeg":
        ...
        data = bytes([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x02, 0x00, 0x00, 0x00, 0x90, 0x77, 0x53, 0xDE, 0x00, 0x00, 0x00, 0x0C, 0x49, 0x48, 0x44, 0x52, 0x08, 0xD7, 0x63, 0x60, 0x60, 0x60, 0x00, 0x00, 0x00, 0x04, 0x00, 0x01, 0x27, 0x34, 0x27, 0x1C, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60, 0x82])
    elif format == "gif":
...
class TestCORS:
    ...
    Acceptance criteria:
      1. OPTIONS preflight from allowed origin → Access-Control-Allow-Origin header.
      2. GET from allowed origin → Access-Control-Allow-Origin header.
      3. Request from disallowed origin → no Access-Control-Allow-Origin header.
      4. Request with credentials → Access-Control-Allow-Credentials: true.
      5. Request with custom header → Access-Control-Allow-Headers includes it.
      6. Existing endpoints continue to function normally.
    """

    def test_options_preflight_returns_cors_headers(self, client):
        """AC 1: OPTIONS preflight from allowed origin returns
    ...
    def test_options_preflight_to_health_endpoint(self, client):
        """AC 1: OPTIONS preflight to /health also returns CORS headers."""
        response = client.options(
            "/health",
            headers={
                "Origin": ALLOWED_ORIGIN,
    ...
    def test_get_from_allowed_origin(self, client):
        """AC 2: GET from allowed origin includes Access-Control-Allow-Origin."""
        response = client.get("/cars", headers={"Origin": ALLOWED_ORIGIN})
        assert response.status_code == 200
        assert response.headers.get("access-control-allow-origin") == ALLOWED_ORIGIN

    def test_get_health_from_allowed_origin(self, client):
        """AC 2: GET /health from allowed origin includes CORS header."""
        response = client.get("/health", headers={"Origin": ALLOWED_ORIGIN})
        assert response.status_code == 200
        assert response.headers.get("access-control-allow-origin") == ALLOWED_ORIGIN

    def test_disallowed_origin_no_cors_header(self, client):
        """AC 3: Request from disallowed origin does NOT include
        Access-Control-Allow-Origin."""
        response = client.get("/cars", headers={"Origin": DISALLOWED_ORIGIN})
        assert response.status_code == 200
        # FastAPI's CORSMiddleware still returns the header but it won't match
        # the disallowed origin — it will NOT include the header at all because
        # the origin is not in allow_origins.
        assert "access-control-allow-origin" not in response.headers

    def test_disallowed_origin_options(self, client):
        """AC 3: OPTIONS from disallowed origin has no CORS header."""
        response = client.options(
            "/cars",
            headers={
    ...
    def test_credentials_header(self, client):
        """AC 4: Response includes Access-Control-Allow-Credentials: true
        when request comes from allowed origin."""
        response = client.get("/cars", headers={"Origin": ALLOWED_ORIGIN})
        assert response.status_code == 200
        assert response.headers.get("access-control-allow-credentials") == "true"

    def test_credentials_header_on_options(self, client):
        """AC 4: OPTIONS preflight includes Access-Control-Allow-Credentials."""
        response = client.options(
            "/cars",
            headers={
                "Origin": ALLOWED_ORIGIN,
    ...
    def test_custom_content_type_header_allowed(self, client):
        """AC 5: Request with Content-Type application/json returns
        Access-Control-Allow-Headers which includes content-type."""
        response = client.options(
            "/cars",
            headers={
                "Origin": ALLOWED_ORIGIN,
        ...
        # Normalise case — FastAPI returns lower-case header names
        assert "content-type" in allow_headers.lower()

    def test_existing_endpoints_still_work(self, client):
        """AC 6: All existing endpoints continue to function normally."""
        ...
        assert response.status_code == 200
        data = response.json()
        assert all(c["brand"] == "Toyota" for c in data["cars"])
...
class TestCreateCar:
    ...
    def test_uploaded_image_served_statically(self, client):
        ...
        # Create a car to trigger the upload
        response = client.post(
            "/cars",
            data={
                "brand": "Audi",
